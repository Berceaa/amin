...
const categories = [
  { to: '/products?category=Dogs', label: 'Dogs', icon: '🐶' },
  { to: '/products?category=Cats', label: 'Cats', icon: '🐱' },
  { to: '/products?category=Cats%20%26%20Dogs', label: 'Cats & Dogs', icon: '🐾' },
];
    ...
    <>
      ...
      <div className="mt-8 border-t border-orange-100 pt-6">
        ...
        <div className="mt-3 space-y-1">
          {categories.map((item) => (
            <NavLink key={item.to} to={item.to} onClick={onClose} className="flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-orange-50 hover:text-[#f27128]">
              <span>{item.icon}</span>
