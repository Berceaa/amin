...
export default function CartDrawer() {
    ...
    return createPortal(
        <>
            ...
            <aside
                className="fixed inset-y-0 right-0 z-[9999] flex h-screen w-full max-w-md flex-col border-l border-orange-100 bg-white shadow-2xl"
                aria-label={t('cart.title') as string}
            >
                <div className="shrink-0 border-b border-orange-100 bg-white px-5 py-4">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#f27128]">
                                Pawsentials
                            </p>
                ...
                {items.length === 0 ? (
                    <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
                        ...
                        <p className="mt-2 text-sm leading-6 text-slate-600">
                            {t('cart.emptyText') as string}
                        </p>

                        <button
                            type="button"
                            onClick={closeCart}
                            className="mt-6 rounded-full bg-[#f27128] px-6 py-3 text-sm font-semibold text-white"
                        >
                            Continue shopping
                        </button>
                    </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-5 py-5">
                            <div className="space-y-4">
                                {items.map((item) => (
                                    <article
                                        key={item.id}
                                        className="rounded-[1.5rem] border border-orange-100 bg-[#fffaf6] p-4"
                                    >
                              ...
                              {item.quantity}
                                                    ...
                                                    <p className="text-base font-black text-[#f27128]">
                                                        {item.price}
                                                    </p>
                                                </div>
                                            </div>
