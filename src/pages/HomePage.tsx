  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        setProducts(result.map(mapProduct).slice(0, 3));
      } catch (error) {
  ...
  return (
      <main>
          ...
          <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 lg:grid-cols-[1.05fr_0.95fr] lg:px-6 lg:py-24">
            <div className="relative">
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <Link
                    to="/products"
                    className="page-link paw-button premium-cta rounded-full bg-[#f27128] px-6 py-3 font-semibold text-white shadow-[0_10px_24px_rgba(242,113,40,0.28)]"
                >
                ...
                <Link
                    to="/contact"
                    className="page-link wag-hover rounded-full border border-orange-200 bg-white px-6 py-3 font-semibold text-slate-800"
                >
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {stats.map((item) => (
                    <div
                        key={item}
                        className="page-link wag-hover rounded-full border border-orange-100 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm"
                    >
                      {item}
                    </div>
                ))}
                ...
                <div className="premium-mascot-card absolute -bottom-2 left-1/2 hidden -translate-x-1/2 items-center gap-3 rounded-[1.6rem] border border-white/70 bg-white/75 px-5 py-3 shadow-[0_18px_40px_rgba(15,23,42,0.16)] backdrop-blur-xl md:flex">
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#f27128]">
                      Pet care concierge
                    </p>
                    <p className="text-sm font-semibold text-slate-700">
                      Premium pickup, grooming and tailored nutrition
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="relative">
              ...
              <div className="pet-float absolute -left-4 top-12 hidden rounded-3xl border border-orange-100 bg-white p-4 shadow-xl lg:block">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#f27128]">
                  {t('hero.cards.delivery') as string}
                </p>
                <p className="mt-2 max-w-[180px] text-sm text-slate-600">
                  {t('hero.cards.repeat') as string}
                </p>
              </div>
              ...
              <div className="pet-float-delay absolute -right-4 bottom-12 hidden rounded-3xl border border-orange-100 bg-white p-4 shadow-xl lg:block">
                ...
                <p className="mt-2 max-w-[180px] text-sm text-slate-600">
                  {t('hero.cards.wellness') as string}
                </p>
              </div>
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {featured.map((item, index) => (
                <InfoCard
                    ...
                    title={item.title}
                    text={item.text}
                    icon={featuredIcons[index % featuredIcons.length]}
                />
            ))}
          </div>
        </ContentSection>
        ...
        <ContentSection title={t('sections.products') as string} description={t('sections.productsDesc') as string}>
          ...
          {productsError && (
              <p className="rounded-2xl border border-red-100 bg-red-50 p-6 text-center font-semibold text-red-600">
                {productsError}
              </p>
          )}
              ...
              <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                {products.map((product) => (
                    ...
                    >
                      ...
                      <div className="p-6">
                        <div className="mb-3 inline-flex rounded-full bg-orange-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[#f27128]">
                          {product.tag}
                        </div>
                    ...
                    <span className="text-lg font-black text-[#f27128]">
                      {product.price}
                    </span>
                          ...
                          <button
                              ...
                              className="paw-button rounded-full border border-orange-200 px-4 py-2 text-sm font-semibold text-slate-800 transition hover:border-orange-300 hover:text-[#f27128] disabled:cursor-not-allowed disabled:opacity-50"
                          >
          ...
          <div className="relative grid gap-10 overflow-hidden rounded-[2rem] bg-[#f27128] px-6 py-10 text-white shadow-[0_30px_70px_rgba(242,113,40,0.35)] lg:grid-cols-[1fr_0.9fr] lg:px-10">
            ...
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.2em] text-orange-100">
                {t('sections.membership') as string}
              </p>
              <h2 className="mt-4 text-3xl font-black leading-tight">
                {t('sections.membershipDesc') as string}
              </h2>
            </div>
            ...
            <div className="space-y-3">
              {membership.map((item) => (
                  <div
                      key={item}
                      className="page-link wag-hover rounded-2xl bg-white/12 px-4 py-3 text-sm font-medium backdrop-blur"
                  >
                    {item}
                  </div>
              ))}
        ...
        <section className="mx-auto my-16 max-w-7xl px-4 lg:px-6">
          <div className="relative overflow-hidden rounded-[2rem] border border-orange-100 bg-white p-8 text-center shadow-sm">
            ...
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#f27128]">
              {t('sections.contact') as string}
            </p>

            <p className="mx-auto mt-4 max-w-2xl text-lg leading-8 text-slate-600">
              {t('sections.contactDesc') as string}
            </p>

            <div className="mt-6 flex flex-wrap justify-center gap-4">
              <Link to="/contact" className="paw-button rounded-full bg-[#f27128] px-6 py-3 font-semibold text-white">
                {t('nav.contact') as string}
              </Link>

              <Link to="/products" className="page-link wag-hover rounded-full border border-orange-200 px-6 py-3 font-semibold text-slate-800">
...
function InfoCard({ title, text, icon }: { title: string; text: string; icon: string }) {
  return (
      <article className="pet-card rounded-[1.75rem] border border-orange-100 bg-white p-6 shadow-sm">
        <div className="pet-icon mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-orange-50 text-2xl">
          {icon}
        </div>
