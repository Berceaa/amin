...
export default function AuthPage() {
  ...
  const validateLoginField = (field: LoginField, value: string): string => {
    if (field === 'email') {
      return validateEmail(value, loginValidationMessages.emailRequired, loginValidationMessages.emailInvalid);
    }
    ...
    switch (field) {
      case 'name':
        return validatePersonName(
            form.name,
            registerValidationMessages.nameRequired,
            registerValidationMessages.nameInvalid,
        );

      case 'email':
        return validateEmail(
            form.email,
            registerValidationMessages.emailRequired,
            registerValidationMessages.emailInvalid,
        );

      case 'phone':
        return validatePhone(
            form.phone,
            registerValidationMessages.phoneRequired,
            registerValidationMessages.phoneInvalid,
        );

      case 'company':
        return validateCompany(
            form.company,
            registerValidationMessages.companyRequired,
            registerValidationMessages.companyInvalid,
        );

      case 'taxNumber':
        return validateTaxNumber(
            form.taxNumber,
            registerValidationMessages.taxRequired,
            registerValidationMessages.taxInvalid,
        );

      case 'password':
        return validatePassword(
            form.password,
            registerValidationMessages.passwordRequired,
            registerValidationMessages.passwordWeak,
        );
  ...
  const validateRegisterForm = (form: RegisterForm) => {
    ...
    return fields.reduce<ValidationErrors<RegisterField>>((acc, field) => {
    ...
    setLoginForm((current) => {
      ...
      if (loginTouched[field]) {
        setLoginErrors((currentErrors) => {
          ...
          return Object.fromEntries(
              Object.entries(nextErrors).filter(([, errorText]) => errorText),
          ) as ValidationErrors<LoginField>;
        });
      }
  ...
  const handleRegisterChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setRegisterForm((current) => {
      ...
      if (registerTouched[field] || (field === 'password' && registerTouched.confirmPassword)) {
  ...
  const handleLoginBlur = (field: LoginField) => {
    ...
    setLoginErrors((current) => {
      const nextErrors = {
        ...current,
        [field]: validateLoginField(field, loginForm[field]),
      };
  ...
  if (user) {
    return (
        <main className="mx-auto max-w-4xl px-4 py-14 lg:px-6">
          <section className="relative overflow-hidden premium-panel rounded-[2rem] border border-orange-100 bg-white p-8 shadow-sm md:p-10">
            <PawTrail className="mb-6" size="sm" />
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              {user.role === 'admin' ? (
                  <Link
                      to="/admin"
                      className="paw-button rounded-full bg-[#f27128] px-6 py-3 font-semibold text-white shadow-[0_10px_25px_rgba(242,113,40,0.28)]"
                  >
                    Admin Page
                  </Link>
              ) : (
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <section className="relative overflow-hidden premium-panel rounded-[2rem] border border-orange-100 bg-white p-8 shadow-sm md:p-10">
            ...
            <div className="mt-8 rounded-[1.5rem] bg-[#fff7f1] p-5 text-sm leading-6 text-slate-600">
              ...
              <p className="mt-2">
                Orders are configured for in-store pickup only, and customer data persists locally in the browser for demo purposes.
              </p>
            </div>
            ...
            {mode === 'register' && (
                <div className="mt-6 rounded-[1.5rem] border border-orange-100 bg-white p-5">
                  <p className="text-sm font-bold text-slate-900">
                    {t('validation.passwordHelperTitle') as string}
                  </p>
          ...
          <section className="relative overflow-hidden premium-panel rounded-[2rem] border border-orange-100 bg-white p-8 shadow-sm md:p-10">
            ...
            {mode === 'login' ? (
                <form onSubmit={handleLogin} noValidate className="space-y-5">
                  ...
                  <button
                      type="submit"
                      className="w-full rounded-full bg-[#f27128] px-6 py-3.5 font-semibold text-white shadow-[0_10px_25px_rgba(242,113,40,0.28)]"
                  >
                ...
                <form onSubmit={handleRegister} noValidate className="grid gap-5 md:grid-cols-2">
                  ...
                  <Input
                      label={t('auth.phone') as string}
                      name="phone"
                      value={registerForm.phone}
                      onChange={handleRegisterChange}
                      onBlur={() => handleRegisterBlur('phone')}
                      error={registerTouched.phone ? registerErrors.phone : ''}
                  />

                  <Input
                      label={t('auth.company') as string}
                      name="company"
                      value={registerForm.company}
                      onChange={handleRegisterChange}
                      onBlur={() => handleRegisterBlur('company')}
                      error={registerTouched.company ? registerErrors.company : ''}
                  />

                  <Input
                      label={t('auth.taxNumber') as string}
                      name="taxNumber"
                      value={registerForm.taxNumber}
                      onChange={handleRegisterChange}
                      onBlur={() => handleRegisterBlur('taxNumber')}
                      error={registerTouched.taxNumber ? registerErrors.taxNumber : ''}
                  />
                  ...
                  <div className="md:col-span-2">
                    <Input
                        ...
                        error={registerTouched.confirmPassword ? registerErrors.confirmPassword : ''}
                    />
                  </div>
            ...
            {error && (
                <p className="mt-4 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm font-medium text-red-600">
                  {error}
                </p>
            )}

            {message && (
                <p className="mt-4 rounded-2xl border border-orange-100 bg-orange-50 px-4 py-3 text-sm font-medium text-[#f27128]">
                  {message}
                </p>
            )}
          </section>
  ...
  return (
      <label className="block text-sm font-semibold text-slate-800">
        ...
        <input
            ...
            className={`w-full rounded-full border bg-[#fffaf6] px-4 py-3 text-sm outline-none transition ${
                error ? 'border-red-300 focus:border-red-400' : 'border-orange-100 focus:border-orange-300'
            }`}
        />
